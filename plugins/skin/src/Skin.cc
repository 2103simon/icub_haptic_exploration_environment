        ...
        for (size_t k=0; k<contacts.contact(j).position_size(); k++)
        {
            ...
            if (linksLocalNames[i] == "r_hand_base_link" || linksLocalNames[i] == "l_hand_base_link")
            {
                ...
                for (size_t m=0; m<taxel_placement_palm.at(linksLocalNames[i])[0].size(); m++)
                {
                    ...
                    double force_tax = force_res * (exp(-((pow( taxel_placement_palm.at(linksLocalNames[i])[1][m]-cont_tip.Pos().X(), 2)/pow(2*sigma, 2)) + (pow( taxel_placement_palm.at(linksLocalNames[i])[2][m]-cont_tip.Pos().Y(), 2)/pow(2*sigma, 2)) + (pow( taxel_placement_palm.at(linksLocalNames[i])[3][m]-cont_tip.Pos().Z(), 2)/pow(2*sigma, 2))))/max_val_gau);
            ...
            else
            {
                ...
                for (size_t m=0; m<taxel_placement_finger.at(linksLocalNames[i])[0].size(); m++)
                {
                    ...
                    // calc force according scaled gaussian
                    double force_tax = force_res * (exp(-((pow( taxel_placement_finger.at(linksLocalNames[i])[1][m]-cont_tip.Pos().X(), 2)/pow(2*sigma, 2)) + (pow( taxel_placement_finger.at(linksLocalNames[i])[2][m]-cont_tip.Pos().Y(), 2)/pow(2*sigma, 2)) + (pow( taxel_placement_finger.at(linksLocalNames[i])[3][m]-cont_tip.Pos().Z(), 2)/pow(2*sigma, 2))))/max_val_gau);
